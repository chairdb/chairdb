...
type BoardEvent = Event<BoardCreated, 'BoardCreated'> | Event<ListAddedToBoard, 'ListAddedToBoard'>
type CardEvent = Event<NewCardAddedToList, 'NewCardAddedToList'> | Event<CardRenamed, 'CardRenamed'> | Event<CardMovedToList, 'CardMovedToList'>
...
export class Board {
    ...
    static reconstitute(events: readonly Event[]): Board | null {
        ...
        const board = applicableEvents.reduce((acc, curr) => acc.withEventApplied(curr), new Board('', []))
...
export class Card {
    ...
    static reconstitute(events: readonly Event<unknown>[]): Card | null {
        const applicableEvents: readonly CardEvent[] = events.filter(Card.isApplicableEvent)
    ...
    static isApplicableEvent(event: Event<unknown>): event is CardEvent {
        return event.name === 'NewCardAddedToList' || event.name === 'CardRenamed' || event.name === 'CardMovedToList'
    }
